import style from './ComProject.module.css'

import IconGreen from '../../assets/lucide_dot.svg';

import IconLink from '../../assets/contact_icons/web.svg'

import IconNext from '../../assets/right.svg'
import IconPrev from '../../assets/left.svg'


import WorkImage1 from "../../assets/Portfolio_images/wa.png";

export const ProjectCard = () => {



    return (
        <>
            <div className='flex flex-row justify-btw items-center'>

                <button>
                    <div className={`flex justify-center items-center ${style.circle}`}>
                        <img src={IconPrev} alt="Previous" />
                    </div>
                </button>

                <div className={`flex flex-row gap-20 ${style.container}`}>
                    <img className={style.imgWork} src={WorkImage1} alt="Image Project 1" />
                    <div className={`flex flex-col justify-start items-start gap-20 ${style.info}`}>

                        <h2>title</h2>
                        <p className='gray'>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua, ut enim.</p>
                        <div className='flex flex-row flex-wrap gap-8 justify-start'>
                            <p className={style.tag}>MongoDB</p>
                            <p className={style.tag}>React</p>
                            <p className={style.tag}>Tailwind</p>
                        </div>
                        <div className='flex flex-row justify-start'>
                            <a href="">

                                <svg
                                    className="icon"
                                    viewBox="0 0 24 24"
                                    xmlns="http://www.w3.org/2000/svg"
                                >
                                    <path
                                        d="M12 2C10.6868 2 9.38642 2.25866 8.17317 2.7612C6.95991 3.26375 5.85752 4.00035 4.92893 4.92893C3.05357 6.8043 2 9.34784 2 12C2 16.42 4.87 20.17 8.84 21.5C9.34 21.58 9.5 21.27 9.5 21V19.31C6.73 19.91 6.14 17.97 6.14 17.97C5.68 16.81 5.03 16.5 5.03 16.5C4.12 15.88 5.1 15.9 5.1 15.9C6.1 15.97 6.63 16.93 6.63 16.93C7.5 18.45 8.97 18 9.54 17.76C9.63 17.11 9.89 16.67 10.17 16.42C7.95 16.17 5.62 15.31 5.62 11.5C5.62 10.39 6 9.5 6.65 8.79C6.55 8.54 6.2 7.5 6.75 6.15C6.75 6.15 7.59 5.88 9.5 7.17C10.29 6.95 11.15 6.84 12 6.84C12.85 6.84 13.71 6.95 14.5 7.17C16.41 5.88 17.25 6.15 17.25 6.15C17.8 7.5 17.45 8.54 17.35 8.79C18 9.5 18.38 10.39 18.38 11.5C18.38 15.32 16.04 16.16 13.81 16.41C14.17 16.72 14.5 17.33 14.5 18.26V21C14.5 21.27 14.66 21.59 15.17 21.5C19.14 20.16 22 16.42 22 12C22 10.6868 21.7413 9.38642 21.2388 8.17317C20.7362 6.95991 19.9997 5.85752 19.0711 4.92893C18.1425 4.00035 17.0401 3.26375 15.8268 2.7612C14.6136 2.25866 13.3132 2 12 2Z"
                                        fill="currentColor"
                                    />
                                </svg>
                            </a>
                            <img className='icon' src={IconGreen} alt="dot" />
                            <a href=""><svg className="icon" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M7.64625 24.3538C7.59976 24.3073 7.56288 24.2522 7.53772 24.1915C7.51256 24.1308 7.49961 24.0657 7.49961 24C7.49961 23.9343 7.51256 23.8692 7.53772 23.8085C7.56288 23.7478 7.59976 23.6927 7.64625 23.6463L22.7925 8.5L11 8.5C10.8674 8.5 10.7402 8.44732 10.6464 8.35355C10.5527 8.25979 10.5 8.13261 10.5 8C10.5 7.86739 10.5527 7.74022 10.6464 7.64645C10.7402 7.55268 10.8674 7.5 11 7.5L24 7.5C24.1326 7.5 24.2598 7.55268 24.3536 7.64645C24.4473 7.74022 24.5 7.86739 24.5 8L24.5 21C24.5 21.1326 24.4473 21.2598 24.3536 21.3536C24.2598 21.4473 24.1326 21.5 24 21.5C23.8674 21.5 23.7402 21.4473 23.6464 21.3536C23.5527 21.2598 23.5 21.1326 23.5 21L23.5 9.2075L8.35375 24.3538C8.30731 24.4002 8.25217 24.4371 8.19147 24.4623C8.13077 24.4874 8.06571 24.5004 8 24.5004C7.93429 24.5004 7.86923 24.4874 7.80853 24.4623C7.74783 24.4371 7.69269 24.4002 7.64625 24.3538Z" fill="currentColor" />
                            </svg></a>
                        </div>


                    </div>

                </div>

                <button className={`flex justify-center items-center ${style.circle}`}>
                    <img src={IconNext} alt="Next" />
                </button>


            </div>
        </>
    );
}
